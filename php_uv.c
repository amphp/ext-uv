...
static php_socket_t php_uv_zval_to_valid_poll_fd(zval *ptr)
{
	...
	if (Z_TYPE_P(ptr) == IS_RESOURCE) {
		if ((stream = (php_stream *) zend_fetch_resource_ex(ptr, NULL, php_file_le_stream()))) {
			...
			if (stream->wrapper) {
				if (!strcmp((char *)stream->wrapper->wops->label, "plainfile") || (!strcmp((char *)stream->wrapper->wops->label, "PHP") && (!stream->orig_path || (strncmp(stream->orig_path, "php://std", sizeof("php://std") - 1) && strncmp(stream->orig_path, "php://fd", sizeof("php://fd") - 1) && strncmp(stream->orig_path, "php://temp", sizeof("php://temp") - 1))))) {
...
#define PHP_UV_FS_SETUP_AND_EXECUTE(command, ...) \
	...
	switch (fs_type) {
		...
		case UV_FS_SENDFILE:
		{
			...
			uv->fs_fd = Z_RES_P(z_outstream);
			++GC_REFCOUNT(uv->fs_fd);
			uv->fs_fd_alt = Z_RES_P(z_instream);
			++GC_REFCOUNT(uv->fs_fd_alt);
...
void static clean_uv_handle(php_uv_t *obj) {
	...
	if (obj->fs_fd != NULL) {
		...
		obj->fs_fd = NULL;
		if (obj->fs_fd_alt != NULL) {
			zend_list_delete(obj->fs_fd_alt);
		}
	}
